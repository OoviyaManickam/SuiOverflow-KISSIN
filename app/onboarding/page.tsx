"use client";
import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import { ConnectButton, useCurrentAccount } from "@mysten/dapp-kit";
import "@mysten/dapp-kit/dist/index.css";
import { KnowledgeLevel, Topic, UserProfile } from "@/types";

const TOPICS: { id: Topic; label: string }[] = [
  { id: "llms", label: "LLMs" },
  { id: "infra", label: "Infra" },
  { id: "startups", label: "Startups" },
  { id: "research", label: "Research" },
  { id: "tools", label: "Tools" },
];

export default function Onboarding() {
  const router = useRouter();
  const account = useCurrentAccount();
  const [level, setLevel] = useState<KnowledgeLevel>("builder");
  const [topics, setTopics] = useState<Topic[]>(["llms"]);

  useEffect(() => {
    if (account?.address) {
      localStorage.setItem("kissin_address", account.address);
    }
  }, [account?.address]);

  function toggleTopic(t: Topic) {
    setTopics((prev) => prev.includes(t) ? prev.filter((x) => x !== t) : [...prev, t]);
  }

  function finish() {
    const address = account?.address;
    if (!address) return;
    const profile: UserProfile = { address, level, topics, createdAt: Date.now() };
    localStorage.setItem("kissin_profile", JSON.stringify(profile));
    localStorage.setItem("kissin_address", address);
    router.push("/");
  }

  return (
    <div className="max-w-md mx-auto px-4 py-16">
      <div className="text-center mb-10">
        <h1 className="text-4xl font-black tracking-tighter mb-2">KISSIN</h1>
        <p className="text-gray-500 text-sm">Keep It Simple or I&apos;ll go INSane</p>
        <p className="text-gray-600 text-xs mt-2">AI noise filter powered by Walrus + Sui</p>
      </div>

      <div className="space-y-6">
        <div>
          <p className="text-gray-400 text-xs tracking-widest uppercase mb-3">Connect Wallet</p>
          <ConnectButton />
          {account?.address && (
            <p className="text-teal-400 text-xs font-mono mt-2">
              Connected: {account.address.slice(0, 6)}...{account.address.slice(-4)}
            </p>
          )}
        </div>

        <div>
          <p className="text-gray-400 text-xs tracking-widest uppercase mb-3">I am a...</p>
          <div className="space-y-2">
            {(["beginner", "builder", "researcher"] as KnowledgeLevel[]).map((l) => (
              <button key={l} onClick={() => setLevel(l)}
                className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition ${level === l ? "bg-teal-400/10 border-teal-400 text-teal-400" : "bg-gray-900 border-gray-800 text-gray-400 hover:border-gray-600"}`}>
                {l === "beginner" && "Beginner — just getting into AI"}
                {l === "builder" && "Builder — using AI in my projects"}
                {l === "researcher" && "Researcher — deep in the field"}
              </button>
            ))}
          </div>
        </div>

        <div>
          <p className="text-gray-400 text-xs tracking-widest uppercase mb-3">I care about...</p>
          <div className="flex flex-wrap gap-2">
            {TOPICS.map((t) => (
              <button key={t.id} onClick={() => toggleTopic(t.id)}
                className={`px-4 py-2 rounded-full border text-sm transition ${topics.includes(t.id) ? "bg-teal-400/10 border-teal-400 text-teal-400" : "bg-gray-900 border-gray-800 text-gray-400 hover:border-gray-600"}`}>
                {t.label}
              </button>
            ))}
          </div>
        </div>

        <button
          onClick={finish}
          disabled={!account?.address || topics.length === 0}
          className="w-full bg-teal-400 text-black font-bold py-3 rounded-xl hover:bg-teal-300 transition disabled:opacity-40 disabled:cursor-not-allowed"
        >
          Start Listening →
        </button>
      </div>
    </div>
  );
}
